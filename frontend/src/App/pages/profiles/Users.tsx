...
import EditUserDetails from './manage/user/EditUserDetails';
      ...
      render: (item) => {
        return (
          <h1 className='flex items-center gap-2'>
            {item.isEmailVerified && item.newEmail && (
              <Tooltip title={`${item.newEmail} need to be verified`}>
    ...
    <div>
      {state?.type === 'new' && (
        <ModalBox
          ...
          title={`Add new User`}
        >
          <AddUser onClose={closeModal} reFetch={reFetchData} />
        </ModalBox>
      )}
      {state?.type === 'details' && (
        <ModalBox
